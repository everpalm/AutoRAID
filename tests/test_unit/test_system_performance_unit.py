...
class TestPerformanceIODepth:
    ...
    def target_performance(self):
        """Fixture to set up the SystemPerformance instance with mocked
        behavior. Auto-runs for each test session.
        """
        # 使用 mock 模擬 perf 類別
    ...
    def test_run_io_operation(self, target_system, rw_mode, io_depth):
        """Parameterized test for running I/O operations with various
        read/write modes and I/O depths. Checks that the run_io_operation
...
class TestPerformanceCPUMask(TestPerformanceIODepth):
    ...
    def test_run_io_operation(self, target_system, rw_mode, cpu_mask):
        ...
        # 檢查 I/O 操作是否被正確執行
        target_system.run_io_operation.assert_called_with(
            rw_mode,
            '4k',
            128,
            1, 30, cpu_mask
    ...
    @pytest.mark.parametrize("rr_table", RR_4K_IOD1_JOB1)
    def test_groupby_rr_mean(self, target_performance, rr_table):
        """Extended test for calculating mean IOPS and BW for read operations
        (RR), using CPU Mask parameter. Verifies mean values meet confidence
        interval.
        """
        # 根據參數決定返回 IOPS 或 BW 的值
        def mock_groupby_io_mean(_io_depth, metric):
