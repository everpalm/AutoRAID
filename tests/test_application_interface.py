...
class TestApplicationInterface(object):
    ...
    def my_app(self, cmdopt):
        ...
        return api(cmdopt.get('mode'), cmdopt.get('if_name'), cmdopt.get('config_file'))
    ...
    def test_command_line(self, my_app, linux_cmd):
        ...
        logger.debug("my_app.command_line = %s, type %s", list_executed, type(list_executed))
        assert list_executed == linux_cmd["Return"][my_app.mode]


# if __name__ == '__main__':
    # pytest.main(['test_application_interface.py', '-s', '-v', '-x'])
    # pytest.main(['test_application_interface.py::TestApplicationInterface', '-s', '-v', '-x'])
    # pytest.main(['test_application_interface.py::TestApplicationInterface::test_command_line', '-s', '-v', '-x'])
    # pytest.main(['-vs','test_application_interface.py::TestApplicationInterface::test_command_line'])
